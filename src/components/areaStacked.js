...
function AreaStackedGraph({seriesData, width, height, colors, darkMode, holdings = [], holdingSymbols = false, stockPage = false}) {
    ...
    const toolTipHandler = (event) => {
        ...
        const adjusted = {};
        const data = seriesData[Math.floor(seriesData.length * (1 - ((localPoint(event).x - margin.left) / xMax)))];
        const index = Math.round(seriesData.length * (1 - ((localPoint(event).x - margin.left) / xMax)));
        let total = 0.0;
    ...
    const calculateColor = (index) => {
        if (parseFloat(seriesData[0][keys[index]]) < parseFloat(seriesData[seriesData.length - 1][keys[index]])) {
            return "url(#red)"
        }
        return "url(#green)"
    }
    ...
    const onPath = (index) => {
        return ((yScale(tooltip.adjusted[keys[index]]) < tooltip.y && yScale(tooltip.adjusted[keys[index + 1]]) > tooltip.y) || (index === keysLen - 1 && yScale(tooltip.adjusted[keys[index]]) < tooltip.y))
    };
    ...
    return (
        <svg width={width} height={height}>
            ...
            <PatternWaves id={'waves'} height={10} width={10} fill="transparent" stroke="white" strokeWidth={2}/>
            <LinearGradient id="green" to="#11998e" from="#38ef7d" fromOffset={0.5}/>
            <LinearGradient id="red" from="#ff4f4f" to="#db3c30" fromOffset={0.5}/>
            <rect x={0} y={0} width={width} height={height} fill={darkMode ? 'url(#radial)' : "transparent"} rx={14}/>
            <Group
                top={margin.top}
                left={margin.left}
            >
                ...
                <AxisLeft //Left
                    ...
                    orientation={"left"}
                    label={"Price (USD)"}
                    labelOffset={margin.right + 15}
                    hideTicks={true}
                    numTicks={Math.floor(yMax / 50)}
                />
                <AreaStack
                    top={margin.top}
                    keys={keys}
                ...
                />
                {
                    ...
                    holdings.map((holding, holdingIndex) => {
                        if (holdingSymbols) {
                            return (
                                <g>
                                    {holding.holdings.map((item, itemIndex) => {
                                        return (
                                            <g>
                                                <GlyphCircle
                                                    left={xScale(getDate(item.purchaseDate))}
                                                    top={yMax}
                                                    fill={item.quantity > 0 ? 'url(#green)' : 'url(#red)'}
                ...
                }
                {
                    <g hidden={hideTooltip}>
                                ...
                                return (
                                    <g key={stock}>
                                        ...
                                        <circle
                                            ...
                                            pointerEvents="none"
                                        />
                                        <rect x={tooltip.x - 90 < 0 ? tooltip.x + 30 : tooltip.x - 95}
                                              y={tooltip.y - 20}
                                              width={75}
                                              height={40}
                                            // fill={"black"}
                                        ...
                                        <Text
                                            ...
                                            fill={"white"}
                                        >
                                            {stock + ' $' + parseFloat(tooltip.data[stock]).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                                        </Text>
                                    </g>
                                );
                            }
                        ...
                        <rect
                            x={tooltip.x + 90 > xMax ? tooltip.x - 85 : tooltip.x - 30 < 0 ? tooltip.x - 15 : tooltip.x - 45}
                            y={yMax + 5}
                            width={90}
                            height={30}
                            stroke={darkMode ? "white" : "black"}
                        ...
                        <Text
                            ...
                            x={tooltip.x + 90 > xMax ? tooltip.x - 40 : tooltip.x - 30 < 0 ? tooltip.x + 30 : tooltip.x}
                            y={yMax + margin.top + 5}
                            width={90}
                            textAnchor="middle"
